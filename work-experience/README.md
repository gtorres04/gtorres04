# Work Experience — Gerlin Orlando Torres Saavedra

> Senior Java Software Engineer · Cartagena de Indias, Bolívar, Colombia

🇬🇧 [English](#english) · 🇪🇸 [Español](#español)

---

## English

### Summary

Software Engineer specialized in backend and distributed architectures, with over a decade implementing critical platforms for banking, investment, electronic payments, and large corporations. I have led and developed solutions based on microservices, hexagonal architecture, and DDD, managing complex domains such as global product hierarchies (Inditex), financial integration ISO-8583/MasterCard (EVOLVE), RedSys PSP/acquiring integration for Bankinter's sandbox, pension funds (SPIRIT, Protección), and high-traffic institutional portals.

My strength lies in designing and building robust, scalable, event-driven services with Java 11–21, Spring Boot 3, integrating ecosystems such as Kafka, PostgreSQL, CosmosDB/Gremlin, MongoDB, or MySQL, and deploying on OpenShift, Kubernetes, or cloud environments (Azure). I complement this with modern engineering practices (CI/CD with GitHub Actions and Jenkins, code review, TDD, E2E testing with Karate, observability with Kibana/Prometheus/Grafana). I have also worked on the frontend with SPAs (React, Angular) and microfrontends.

As a technical leader I have guided multidisciplinary teams, defining architecture, reviewing design, raising quality standards, mentoring developers, and ensuring deliverables aligned with business, regulatory, and security requirements. I have also worked as an independent freelancer, building polyglot microservices in Java/Vert.x and Go for the telco sector (MasOrange), with Cadence workflow orchestration, Kafka, PostgreSQL, and Redis on Google Cloud Platform. In recent engagements, I have used Claude Code at MasOrange and GitHub Copilot and OpenCode at Globant for Inditex, with Copilot integrated into the corporate SDD workflow and harnesses configured to direct LLM-assisted development while retaining technical ownership. I am passionate about high-performance systems, reactive flows, and continuous learning: currently deepening my knowledge in Kotlin, Rust, Quarkus, columnar databases, and workflow orchestration with Cadence.

My focus is on creating clear, maintainable, and resilient software that generates real business impact.

---

## Español

### Extracto

Soy Ingeniero de Software especializado en backend y arquitecturas distribuidas, con más de una década implementando plataformas críticas para banca, inversión, pagos electrónicos y grandes corporaciones. He liderado y desarrollado soluciones basadas en microservicios, arquitectura hexagonal y DDD, gestionando dominios complejos como jerarquías globales de producto (Inditex), integración financiera ISO-8583/MasterCard (EVOLVE), integración RedSys PSP/adquirente para el sandbox de Bankinter, fondos de pensiones (SPIRIT, Protección) y portales institucionales de alto tráfico.

Mi fortaleza está en el diseño y construcción de servicios robustos, escalables y event-driven con Java 11–21, Spring Boot 3, integrando ecosistemas como Kafka, PostgreSQL, CosmosDB/Gremlin, MongoDB o MySQL, y desplegando en OpenShift, Kubernetes o entornos cloud (Azure). Complemento este trabajo con prácticas modernas de ingeniería (CI/CD con GitHub Actions y Jenkins, code review, TDD, pruebas E2E con Karate, observabilidad con Kibana/Prometheus/Grafana). También he trabajado en frontend con SPA (React, Angular) y microfrontends.

Como líder técnico he guiado equipos multidisciplinares, definiendo arquitectura, revisando diseño, elevando estándares de calidad, acompañando a desarrolladores y asegurando entregables alineados con negocio, normativa y seguridad. También he trabajado como freelance independiente, construyendo microservicios políglotas en Java/Vert.x y Go para el sector telco (MasOrange), con orquestación de flujos de trabajo con Cadence, Kafka, PostgreSQL y Redis sobre Google Cloud Platform. En proyectos recientes he utilizado Claude Code en MasOrange y GitHub Copilot y OpenCode en Globant para Inditex, con Copilot integrado en el flujo corporativo de SDD y harnesses configurados para dirigir el desarrollo asistido por LLM, conservando la responsabilidad técnica. Me apasionan los sistemas de alto rendimiento, los flujos reactivos y el aprendizaje continuo: actualmente profundizo en Kotlin, Rust, Quarkus, bases de datos columnares y orquestación de flujos con Cadence.

Mi foco es crear software claro, mantenible y resiliente que genere impacto real en el negocio.

---

For the full work experience and education details, browse the directories below.

---

## Experience Index / Índice de Experiencias

| Company | Role | Period |
|---|---|---|
| [MasOrange (Freelance)](masorange/README.md) | Senior Backend Engineer (Java/Go) | Jan 2026 - Mar 2026 |
| [Globant](globant/README.md) | Senior Java Software Engineer | 2023 - Present |
| [Paradigma Digital](paradigma-digital/README.md) | Senior Software Development Engineer | 2021 - 2024 |
| [Grupo Ditech](grupo-ditech/README.md) | Java Architect | 2019 - 2021 |
| [Makers Solutions S.A.S](makers-solutions/README.md) | Java Technical Lead | 2018 - 2019 |
| [Personalsoft S.A.S](personalsoft/README.md) | Software Engineering Analyst | 2017 - 2018 |
| [integ.ro](integro/README.md) | Senior Java Developer | 2017 |
| [everis](everis/README.md) | Senior Solutions Analyst | 2016 - 2017 |
| [PRAGMA S.A.](pragma/README.md) | Software Developer | 2016 |
| [ingemedic & Metrologia](ingemedic-metrologia/README.md) | Java Developer Analyst | 2013 - 2015 |
| [Fundación Universitaria Tecnológico Comfenalco](comfenalco/README.md) | Faculty / Junior Developer | 2008 - 2015 |

### Independent Projects / Proyectos Independientes

| Project | Focus | Period |
|---|---|---|
| [Fund Custard](independent-projects/fund-custard/README.md) | Collective savings fund platform | Not specified |

---

← Back to [main profile](../README.md)
